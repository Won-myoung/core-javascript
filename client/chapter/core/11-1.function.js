...
function rem(pxValue, base = 16){
    ...
    // if(typeof base === 'string'){ pxValue = parseInt(base,10); }
...
function getStyle(node,prop){  //node,prop
...
function setStyle(node,prop,value){
  ...
  if(typeof prop !== 'string'){
    throw new Error('setStyle 함수의 두 번쨰 인수는 문자 타입 이어야 합니다.')
  }

  if(!value) throw new Error('setStyle 함수의 세 번쨰 인수는 필수 입력값 입니다.')

  node.style[prop] = value     //점표기법/대괄호표기법 각각적용해보기
...
// function css(node,prop,value) => (!value) ? getStyle(node,prop) : setStyle(node,prop,value);
